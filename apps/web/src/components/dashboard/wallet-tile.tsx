...
export function WalletTile({
  ...
  return (
    <div className="group relative overflow-hidden rounded-2xl border border-success/30 bg-success/10 p-5 transition-colors hover:border-success/50">
      <Link href={`/${locale}/rewards`} className="block">
        ...
        {isFresh ? (
          <>
            ...
            <p className="mt-1 text-xs text-fg-muted">
              +200 SA per phase · +1,000 SA on grand test · +1,000 SA per referral. Redeem up to
              100% on your next cohort.
            </p>
          </>
        ) : (
          <>
      ...
      {available > 0 && (
        <Link
          ...
          className="mt-3 inline-flex items-center gap-1 font-mono text-[10px] uppercase tracking-wider text-fg-muted hover:text-fg"
        >
